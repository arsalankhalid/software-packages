...
public class Graph {
  ...
  The value property represents the set of string which designate upon which 
  vertex the vertex or key it depends on! Therefore: Value=set of Vertex strings, Key=Dependency
  */
  ...
  /* 
  addEdge() method gets the names of the desired vertex and the vertex to link to
  through the parameters, then adds both of those vertices to the list of known 
  vertices (if they haven't been added before), and adds the edge between them.
  */
	public void addEdge(String rootVertex, String vertexToDependOn){
		...
		if(allVertices.containsKey(rootVertex)){
			...
			if(isEdge(rootVertex, vertexToDependOn)){
				System.out.println("~~~Issue: " +rootVertex +" --> " +vertexToDependOn +". Edge already exists~~~");
			}
			//if vertex does exists, add the vertexToDependOn and place an edge between the two
			else{
				//get all root vertex's into a set, then add an edge to the vertex
				...
				//adds the vertexToDependOn to the allVertices and an edge between the passed in root vertex and vertexToDependOn
				createVertexIfNotExists(vertexToDependOn, null);
				System.out.println(rootVertex + " --> " +vertexToDependOn +". edge has been added");
			}
		}
		else{
			//if the root vertex and vertex to depend on don't exist create them and add an edge between them
			createVertexIfNotExists(rootVertex, vertexToDependOn);
			//debug note
			System.out.println(rootVertex +" ===> " +vertexToDependOn +". This edge has been added");
		}
	}
	
  /*
  Method gets two vertices and  
  adds both of them to allVertices, then adds an edge 
  between them if the vertexToDependOn argument isn't passed as null
  */
	
	private void createVertexIfNotExists(String newVertex, String vertexToDependOn){
		...
		if(!allVertices.containsKey(newVertex)){
			...
			//If the second argument (vertexToDependOn isn't null, then add to the tree, and 
  ...
  //Checks for an edge between 2 passed in vertex substrings
	public boolean isEdge(String rootVertex, String vertexToDependOn){
		boolean result = false; //default set to edges between passed in vertices
		
		//get all vertices with the root vertex name 
		TreeSet<String> verticeSet = allVertices.get(rootVertex);
		
		//check if the set contains the vertex to depend on as a key, if it does then
		//there's an edge
  ...
  //get number of edges by checking each vertex for a key that matches another vertex using an iterator
	public int getNumberOfEdges(){
		int numOfEdges = 0;
		//create a generic collection holding all the vertex's (all the vertexValues)
		...
		while(vertexValuesIter.hasNext()){	 
			  ...
			  TreeSet<String> verticeSet = (TreeSet<String>) vertexValuesIter.next();
	...
	public void printEdges(String vertex1, String vertex2){
		...
		while(allKeys.hasNext()){	 
			  ...
			  TreeSet<String> mainVertices = (TreeSet<String>) allVertexValues.next();
			  if(keyName.contains(vertex1)){
				  Iterator<String> mainVerticesIter = mainVertices.iterator();
				  while(mainVerticesIter.hasNext()){
					  String vertexToDependOn = mainVerticesIter.next().toString();
					  if(vertexToDependOn.contains(vertex2)){
					  	System.out.println("Found an edge between: " + keyName + "and" + "--> " + vertexToDependOn);
            }
				  }
			  }
		}
		System.out.println("The edge between: " +vertex1 +" and " +vertex2 +" was not found");
	}
	...
	public void printPath(String vertex1, String vertex2, int pathLength){
		...
		while(allVerticesIter.hasNext()){
			...
			if(vertexName.contains(vertex1)){
				int pathCounter = 0;
				String matchedPath = depthFirstPathSearch(vertexName,vertex2,pathCounter,pathLength, pastVertices);
				if(matchedPath!="")
					System.out.println(matchedPath);
				else
					System.out.print("The path of length " +pathLength +" between '" +vertex1
							+"' and '" +vertex2 +"' has not been found\n");
				return;
			}
		}
	}
	
	private String depthFirstPathSearch(String currentVertex, String searchVertex, int pathCounter, int pathLength, 
		...
		if(pathCounter>=pathLength){
			...
			while(mainVertecesIter.hasNext()){
				...
				String matchedPath = depthFirstPathSearch(currentVertexLeaf,searchVertex,pathCounter,pathLength,pastVertices);
				if(matchedPath!=""){
